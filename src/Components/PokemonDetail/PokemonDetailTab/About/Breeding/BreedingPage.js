...
const BreedingPage = () => {
  const { pokemonAbout, detailPokemon } = useContext(PokedexContext);
  ...
  const inches = (detailPokemon.height * 3.93701).toFixed(0);
  const feet = Math.floor(Number(inches) / 12);

  //Get Gender
  const genderPercentage =
    pokemonAbout.gender_rate !== -1 ? (pokemonAbout.gender_rate / 8) * 100 : -1;
    ...
    <section className="text-base">
      ...
      <article className="w-11/12 mx-auto my-4 rounded-xl px-6 py-4 flex justify-center items-center shadow-lg border-gray-100 border">
        ...
        <div className="w-1/2 text-center">
          ...
          <p>{Math.abs(detailPokemon.weight / 4.536).toFixed(1)} lbs</p>
        </div>
      </article>
          ...
          {genderPercentage === -1 ? (
            ...
            <>
              <figure className="flex items-center justify-center mx-3">
                <img className="w-5 h-5" src={Male} alt="Male symbol" />
                <figcaption className="pt-1.5 pl-2">
                  {100 - genderPercentage}%
                </figcaption>
              </figure>
              <figure className="flex items-center justify-center mx-3">
                <img className="w-5 h-5" src={Female} alt="Female symbol" />
                <figcaption className="pt-1.5 pl-2">
                  {genderPercentage}%
                </figcaption>
              </figure>
